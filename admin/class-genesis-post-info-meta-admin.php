 ...
 * @subpackage Genesis_Post_Info_Meta/admin
 */
 ...
 */
class Genesis_Post_Info_Meta_Admin {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Genesis_Post_Info_Meta_Admin    $instance    The instance of this class.
	 */
     ...
     *
     * @return    Genesis_Post_Info_Meta_Admin    A single instance of this class.
     */
	...
	public function dismiss_notice() {
		...
		if ( isset( $_GET['gpim_notice_ignore'] ) && '0' == $_GET['gpim_notice_ignore'] ) {
			add_user_meta( $user_id, 'gpim_notice_ignore', true, true );
		}
	...
	public function admin_notices() {
		...
		// Do activation notice.
		$this->do_activation_notice();

		// Do Genesis notice.
		$this->do_genesis_notice();

	}

	/**
	 * Do notice upon activation.
	 *
	 * @since    1.0.0
	 */
	...
	/**
	 * Do notice if Genesis is not active.
	 *
	 * @since    1.0.0
	 */
	public function do_genesis_notice() {

		if ( false === strpos( wp_get_theme()->Template, 'genesis' ) ) {

			$notice_message = sprintf(
				__( '%s This plugin requires the Genesis Framework to function properly. Please %sactivate a Genesis theme%s to use this plugin.', 'genesis-post-info-meta' ),
				'<b>' . $this->plugin_name . ':</b>',
				'<a href="' . get_admin_url( null, 'themes.php' ) . '">',
				'</a>'
			);

			echo '<div class="updated error"><p>' . $notice_message . '</p></div>';
	...
	public function register_customizer_controls( $wp_customize ) {
		...
		// Exit if Genesis isn't active.
		if ( false === strpos( wp_get_theme()->Template, 'genesis' ) ) {
			return;
		}

		// Get main option.
		$plugin_option = $plugin_option_orig = get_option( $this->plugin_slug );

		/**
		 * Add custom customizer controls.
		 */
		if ( class_exists('WP_Customize_Control') ) {
			gpim_add_custom_controls();
		}
		...
		$wp_customize->add_panel(
			...
			array(
			    ...
			    'description' => __( 'Choose where to display post info and meta.', 'genesis-post-info-meta' ),
			)
		);

		// Set up setting type array.
		$setting_types = array(
			'info' => __( 'Post Info', 'genesis-post-info-meta' ),
			'meta' => __( 'Post Meta', 'genesis-post-info-meta' ),
		);

		// Get post types to loop through.
		$args = array(
			'public' => true,
		);
		$post_types = get_post_types( $args, 'objects' );

		// Exclude pages since they don't get post info/meta.
		unset( $post_types['page'] );

		// Loop through output of settings.
		foreach ( $setting_types as $setting_type => $setting_label ) {

			// Add settings section.
			$wp_customize->add_section(
				"genesis-post-info-meta-section-{$setting_type}",
				array(
					'title'       => $setting_label,
					'description' => sprintf( esc_html__( 'Check the boxes next to the post types for which you wish to show %s.', 'genesis-post-info-meta' ), $setting_label ),
					'priority'    => 5,
					'panel' => $this->plugin_slug,
				)
			);
			...
			foreach ( $post_types as $post_type ) {
				...
				$wp_customize->add_setting(
					"{$this->plugin_slug}[post_{$setting_type}_heading]",
					array(
						'type'    => 'option',
					)
				);
				...
				// Add setting.
				$wp_customize->add_setting(
					"{$post_type->name}_{$setting_type}_heading",
					array(
						'type'    => 'option',
					)
				);
				...
				$wp_customize->add_control(
					...
					array(
						...
						'section'  => "genesis-post-info-meta-section-{$setting_type}",
						'settings' => "{$post_type->name}_{$setting_type}_heading",
						'type'     => 'hidden',
					)
				);

				/**
				 * Add setting and control for single and archive.
				 */
				$display_types = array(
					'single' => __( 'Single', 'genesis-post-info-meta' ),
					'archive' => __( 'Archive', 'genesis-post-info-meta' ),
				);

				foreach ( $display_types as $display_type => $display_type_label) {

					$control_name = "{$post_type->name}_{$setting_type}_{$display_type}";
					$setting_name = "{$this->plugin_slug}[$control_name]";

					// Initialize option default if necessary.
					if ( ! isset( $plugin_option[ $control_name ] ) ) {
						$plugin_option[ $control_name ] = true;
					}
		...
		} // End $setting_types loop

		// Update option with defaults if needed.
		if ( $plugin_option != $plugin_option_orig ) {
			update_option( $this->plugin_slug, $plugin_option );
		}
...
function gpim_add_custom_controls() {

	class WP_Customize_Hr_Control extends WP_Customize_Control {
        /**
